...
{
	private final static Collection<Coeffs> _coeffs = new ArrayList<Coeffs>();
	Coeffs[] ta =
	{
	...
	private static Difference compare(final Step alpha, final Step beta)
	{
		...
		if (beta.getId() == alpha.getId()) return Difference.Equal;
	...
	public static List<Example> filter(final List<Example> examples, final Example example)
	{
		...
		final List<Example> out = new ArrayList<Example>();
